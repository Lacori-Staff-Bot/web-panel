...
function Restrictions({ cookies, removeCookies, notify, setNotify }) {
    ...
    useEffect(() => {
        restrictions(cookies.auth, cookies.key, params.id, "get_info", setResponse, removeCookies, notify, setNotify, navigate);
    }, [navigate, setResponse, params.id, cookies.auth, cookies.key, removeCookies, notify, setNotify]);
    ...
    return (
        <div className="mainBlock">
            {response !== null ? (<><div className="block" key={"settings"}>
                ...
                <div className="saveButtonBlock" key={"save_button"}>
                    <MyButton onClick={(ev) => {
                        if (!(0 <= maxBans && maxBans <= 10)) {
                            setNotify(notify => [...notify, <Notify label={"Ошибка"} type={"Error"} description={"Максимальное количество банов должно быть в диапазоне от 0 до 10."} key={notify.length} />]);
                        }
                        if (!(0 <= maxMutes && maxMutes <= 15)) {
                            setNotify(notify => [...notify, <Notify label={"Ошибка"} type={"Error"} description={"Максимальное количество мутов должно быть в диапазоне от 0 до 15."} key={notify.length} />]);
                        }
                        if (!(0 <= maxWarns && maxWarns <= 15)) {
                            setNotify(notify => [...notify, <Notify label={"Ошибка"} type={"Error"} description={"Максимальное количество варнов должно быть в диапазоне от 0 до 15."} key={notify.length} />]);
                        }
                        if (!(0 <= maxPreds && maxPreds <= 20)) {
                            setNotify(notify => [...notify, <Notify label={"Ошибка"} type={"Error"} description={"Максимальное количество предов должно быть в диапазоне от 0 до 20."} key={notify.length} />]);
                        }
                        ...
                        restrictions(cookies.auth, cookies.key, params.id, "set_settings", undefined, removeCookies, notify, setNotify, navigate, signalChannel, maxBans, maxMutes, maxWarns, maxPreds, parsedPermissions)
