...
function activeBans(cookie, key, params, type, id, setResponse, removeCookies, setNotify, navigate) {
    if (!cookie) {
    ...
    sendAPIRequest("POST", "/active_bans", data, (ev) => {
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                ...
                setNotify(<Notify label={"Ошибка"} type={"Error"} description={"Бан уже не является активным."} />)
            }
        }
    })
}
